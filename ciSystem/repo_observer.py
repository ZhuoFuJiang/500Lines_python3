...
def poll():
    ...
    parser.add_argument("--dispatcher-server",
                        help="dispatcher host:port by default "
                             "it uses localhost:8888",
                        default="localhost:8888",
                        action="store")
    parser.add_argument("repo", metavar="REPO", type=str,
                        help="path to repository this will observe")
    ...
    while True:
        ...
        if os.path.isfile(".commit_id"):
            ...
            if response == "OK":
                ...
                print("dispatched test task: %s" % commit)
            else:
                raise Exception("Could not dispatch the test %s: %s" % (commit, response))
